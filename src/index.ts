...
let videos = videoFiles.map((file) => {
  ...
  return {
    ...
    path: path.join(config.videosFolder, file),
  };
});
...
streamer.client.on("ready", () => {
  if (streamer.client.user) {
    ...
    streamer.client.user.setActivity(
      status_idle() as unknown as ActivityOptions
    );
  }
});
  ...
  if (config.cmdChannelId.includes(message.channel.id)) {
    switch (user_cmd) {
      case "play":
        ...
        streamer.client.user?.setActivity(
          status_watch(videoname) as unknown as ActivityOptions
        );
        break;
      case "playlink":
        console.log("XD")
        ...
        switch (true) {
          case ytdl.validateURL(link):
            ...
            if (yturl) {
              ...
              streamer.client.user?.setActivity(
                status_watch("") as unknown as ActivityOptions
              );
            }
            break;
          default:
      ...
      case "cuevanaserie":
        ...
        try {
          const m3u8Url = await getM3U8FromCuevana(cuevanaUrl);
          ...
          playVideo(m3u8Url, streamLinkUdpConn);
      ...
      case "cuevanapeli":
        ...
        try {
          ...
          playVideo('https://be4235.rcr32.ams02.cdn112.com/hls2/03/06649/n4b85f8ilomg_h/master.m3u8?t=9RFCxbxTE7dXcRdu7BEBfflIeWWN_Ta3Zkhuz39jE9k&s=1723362383&e=10800&f=33430965&srv=31&asn=271835&sp=4000', streamLinkUdpConn);
          message.reply("**Playing video from Cuevana...**");
          console.log("Playing video from Cuevana...");
          streamer.client.user?.setActivity(
            status_watch("Cuevana Video") as ActivityOptions
      ...
      case "preview":
        ...
        try {
          const thumbnails = await ffmpegScreenshot(
            `${vid_name.name}${path.extname(vid_name.path)}`
          );
      ...
      case "help":
        const commands = {
          ...
          playlink: {
            description:
              "Play a video/video/stream direct link or from youtube link",
            usage: "playlink [link]",
          },
          ...
          preview: {
            description:
              "Generate and obtain preview thumbnails of a specific video.",
            usage: "preview [video name]",
          },

          cuevanaserie: {
            description: "Play a video from Cuevana",
            usage: "cuevana [cuevana url]",
          },
...
async function sendFinishMessage() {
  const channel = streamer.client.channels.cache.get(
    config.cmdChannelId.toString()
  ) as TextChannel;
